...
class S_x:
    ...
    def __init__(self, A, X):
        ...
        self.res_per_route = [0,0]*len(A) # how much capacity taken by each type of route in *current*

    def get_next(self):
        if self.one_more_route_i_fits(self.route):
            self.rem1 -= self.A[self.route][0]
            self.rem2 -= self.A[self.route][1]
            self.current[self.route] += 1
            self.res_per_route[self.route][0] += self.A[self.route][0]
            self.res_per_route[self.route][1] += self.A[self.route][1]
            return self.current
        else:
